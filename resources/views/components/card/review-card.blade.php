<div class="mx-auto bg-white rounded-xl shadow-lg overflow-hidden p-6">
    ...
    <!-- Comment Image -->
    <a href="{{ Route('restaurant.index', ['slug' => Str::slug($restaurantName)]) }}" class="font-semibold mb-3 hover:text-primary">{{ $restaurantName }}</a>
    <img class="w-full h-48 object-cover rounded-lg mb-3" src="https://iber.or.id/wp-content/themes/consultix/images/no-image-found-360x250.png" alt="Food Image">
    ...
    <div class="flex items-center justify-between gap-2">
        ...
        <div>
            <button data-dropdown-toggle="dropdownReview{{ $commentId }}" data-dropdown-placement="bottom-end" class="hover:rounded-full hover:bg-gray-200 hover:text-primary py-0.5 px-1.5"><i class="ri ri-more-2-line hover:text-primary text-2xl"></i></button>
        </div>
    </div>
</div>

<!-- Dropdown menu for each comment -->
<div id="dropdownReview{{ $commentId }}" class="z-10 hidden bg-secondary-background divide-y divide-gray-100 rounded-lg shadow-sm w-36">
    <ul class="py-2 text-sm text-black" aria-labelledby="dropdownReviewButton">
        <li>
            <a href="{{ Route('restaurant.index', ['slug' => Str::slug($restaurantName)]) }}" class="px-4 py-2 hover:!text-white hover:bg-primary flex items-center">
