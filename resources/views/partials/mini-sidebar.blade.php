    ...
    <div class="w-full h-full py-4 overflow-y-auto bg-transparent">
        <ul class="space-y-2 font-medium">
            ...
            @if (Auth::user()->hasRole('admin'))
                <x-nav-item href="{{ route('admin.dashboard.index') }}" icon="ri-dashboard-line" active="{{ request()->routeIs('admin.dashboard.index') }}">
                    Dashboard
                </x-nav-item>
                <x-nav-item href="{{ route('admin.user.index') }}" icon="ri-user-line" active="{{ request()->routeIs('admin.user.index') }}">
                    Manage Users
                </x-nav-item>
                <x-nav-item href="{{ route('admin.restaurant.index') }}" icon="ri-restaurant-line" active="{{ request()->routeIs('admin.restaurant.index') }}">
                    Manage Restaurants
                </x-nav-item>
            @else
