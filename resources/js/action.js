    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status && response.message) {
                $(e.target).before(
                    generateAlertHtml("success", response.message)
                );
            }
            ...
            $.each($(e.target).data(), function (key, value) {
                if (typeof window[value] === "function") {
                    try {
                        ...
                        console.error(
                            `Error saat memanggil fungsi dari data-${key}:`,
                            err
                        );
                    }
                }
        ...
        error: function (xhr) {
            ...
            if (
                ...
                if (response.errors) {
                    $(e.target).before(
                        generateAlertHtml("danger", response.errors)
                    );
                } else {
                    $(e.target).before(
                        generateAlertHtml("danger", response.message)
                    );
                }
            }
        },
...
function generateAlertHtml(type, message) {
    ...
    const alertClasses =
        type === "success"
            ...
            : type === "info"
            ? "bg-blue-50 text-info border-info"
